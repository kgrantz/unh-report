...
```{r isol-quar-projections}
  ...
  to_add_quar <- to_add_quar %>%
      filter(off_campus_fac=="Total" & scaling_fac==2) %>%
      mutate(scaling_fac = "2 w/o off-campus") %>%
      bind_rows(to_add_quar) 
  ...
  ts_quar_proj <- ts_quar_proj %>%
        filter(off_campus_fac=="On Campus" & scaling_fac==2) %>%
        mutate(off_campus_fac="Total") %>%
        mutate(scaling_fac = "2 w/o off-campus") %>%
        bind_rows(ts_quar_proj)
...
```{r Table-B1A-StateCond}
  ...
  state_curr_hosp <- as.numeric(gsub(",", "", covid_num[5]))
  ...
  state_tab <- data.frame(var = c("Current COVID-19 cases",
                          ...
                          level = c(pick_color_threshold_str(state_curr_cases, thresholds=c(0, 100, 3000, 6000)),
                                    pick_color_threshold_str(state_curr_hosp, thresholds = c(0, 25, 200, 400)),
                                    "link"))
  
  cell_cols <- c(pick_color_threshold(state_curr_cases, thresholds=c(0, 100, 3000, 6000)),
                 pick_color_threshold(state_curr_hosp, thresholds = c(0, 25, 200, 400)),
                 "#FFFFFF")
  state_tab %>%
    flextable() %>%
    ...
    bg(j=3, bg = cell_cols) %>%
    compose(i=3, j=3, value=as_paragraph("More info: ", hyperlink_text(x="www.nh.gov/covid19", url="https://www.nh.gov/covid19"))) %>%
    autofit() %>%
    add_footer_lines(values = glue("{statedatetimeupdated}")) %>%
    bold(i=1, part="header") %>%
    set_caption(caption = glue("Table B{tab_counter}. COVID-19 conditions in New Hampshire."))
...
```{r Table-B1B-CampusCond}
  ...
  cap <- glue("Table {tab_counter}. Key indicators at each campus. Select indicators are shaded by their operational level.")
  tab_counter <- tab_counter + 1
  ...
  isoldf <- ts_isol_campus %>%
            ...
            replace_na(list(NAvgIsol = 0,
                            NAvgIsolBed = 0)) %>%
            mutate(NAvgIsolBedPct = NAvgIsolBed / isol_bed_limit) %>%
  ...
  quardf <- ts_quar_campus %>%
            ...
            replace_na(list(NAvgQuar = 0,
                            NAvgQuarBed = 0)) %>%
            mutate(NAvgQuarBedPct = NAvgQuarBed / quar_bed_limit) %>%
  ...
  indicatordf <-  left_join(testdf,casedf,by="campus_fac")  %>%
                  ...
                  filter(campus_fac %in% c("UNH Durham","UNH Manchester", "UNH Law")) %>%
  ...
  active_case_colors <- indicatordf %>%
                        ...
                        select(-Indicator) %>%
                        mutate_all(function(x){pick_color_threshold(as.numeric(x), thresholds=c(-1, 20, 150, 300))}) %>%
                        unlist()
  ...
  pct_pos_colors <- posdf %>%
                    ...
                    arrange(campus_fac) %>%
                    group_by(campus_fac) %>%
                    mutate(col=pick_color_threshold(as.numeric(pct_pos), thresholds=c(-1, 0.0025, 0.01, 0.07))) %>%
                    ungroup() %>%
                    select(col) %>%
                    unlist()

  isol_bed_color_unh <- isoldf %>%
                        filter(campus_fac=="UNH Durham") %>%
                        mutate(col = pick_color_threshold(as.numeric(NAvgIsolBedPct)*100, thresholds=c(-1, 10, 50, 90))) %>%
                        ungroup() %>%
                        select(col) %>%
                        unlist()

  quar_bed_color_unh <- quardf %>%
                        filter(campus_fac=="UNH Durham") %>%
                        mutate(col = pick_color_threshold(as.numeric(NAvgQuarBedPct)*100, thresholds=c(-1, 25, 50, 90))) %>%
                        ungroup() %>%
                        select(col) %>%
                        unlist()
  ...
  indicatordf %>% 
    ...
    bg(i=~(Indicator == "Average isolation beds in use (n, %)"), j=2, bg = isol_bed_color_unh) %>%
    bg(i=~(Indicator == "Average quarantine beds in use (n, %)"), j=2, bg = quar_bed_color_unh) %>%
    autofit() 
...
```{r Figure-B1-EpiCurve, fig.cap = cap}
  ...
  ggplot(ts_cases_campus, aes(x=testdate, y=n, fill=campus_fac)) +
    ...
    scale_x_date(name="", date_breaks = "1 week", limits = xaxis_entire) +
...
```{r Figure-B2-EpiCurve-7day, fig.cap = cap}
  ...
  labs <- ts_cases_campus %>% 
          filter(testdate <= report_date, testdate > report_date_minus7) %>%
          group_by(testdate) %>%
          summarise(n=sum(n))

  ggplot(ts_cases_campus %>% filter(testdate <= report_date, testdate > report_date_minus7), aes(x=testdate, y=n, fill=campus_fac)) +
    geom_col(width=0.9)+
    geom_text(data=labs, aes(x=testdate, y=n+2, label=n, fill=NA)) +
    theme_bw()+
    scale_fill_brewer("Campus",type = "qual") +
    scale_y_continuous(name = "Cases", breaks=breaks_pretty()) +
...
```{r Figure-B4-Quar-7day, fig.cap=cap}
  ...
  ts_quar_role_labs <- ts_quar_role_sum %>%
                       group_by(date) %>%
                       summarise(n = sum(n))
  
  ## number in quarantine, by role. by day, last 7 days
  ggplot(ts_quar_role_sum %>% filter(date <= report_date, date > report_date_minus7), aes(x=date, y=n)) +
    geom_col(aes(fill=user_status_fac)) +
    geom_text(data=ts_quar_role_labs, aes(x=date, y=n+20, label=n)) +
    theme_bw()+
...
```{r Figure-B6-Isol-7day, fig.cap=cap}
  ...
  ts_isol_role_labs <- ts_isol_role_sum %>%
                       group_by(date) %>%
                       summarise(n = sum(n))
  
  ggplot(ts_isol_role_sum %>% filter(date <= report_date, date > report_date_minus7), aes(x=date, y=n)) +
    geom_col(aes(fill=user_status_fac)) +
    geom_text(data=ts_isol_role_labs, aes(x=date, y=n+5, label=n)) +
    theme_bw()+
    scale_fill_brewer("",type = "qual") +
    scale_y_continuous(name = "", breaks=breaks_pretty()) +
...
```{r Table-C1-DurhamCond}
  ...
  # This Week value + Threshold Indicator, if applicable
    ...
    cap <- glue("Table C{tab_counter}. Durham Campus Operating Conditions. Select indicators are shaded by their operational level.")
    tab_counter <- tab_counter + 1 
...
```{r Figure-C2-EpiCurve-7day, fig.cap=cap}
  ...
  labs <- ts_cases_role %>% 
          filter(campus_fac == "UNH Durham" & testdate <= report_date & testdate > report_date_minus7) %>%
          group_by(testdate) %>%
          summarise(n=sum(n))
  
  ## epi curve, new positive, last 7 days, by day, colored by role
  ggplot(ts_cases_role %>% filter(campus_fac == "UNH Durham" & testdate <= report_date & testdate > report_date_minus7), aes(x=testdate, y=n)) +
    geom_col(aes(fill=user_status_fac), width=0.9)+
    geom_text(data=labs, aes(x=testdate, y=n+2, label=n, fill=NA)) +
    theme_bw()+
    scale_fill_brewer("Campus",type = "qual") +
    scale_y_continuous(name = "Cases", breaks=breaks_pretty()) +
...
```{r Table-C5-Lab}
  ...
  cap <- glue("Table C{tab_counter}. Laboratory operations: number and percent (of all tests submitted) of tests returned with results at UNH Durham in the reporting period and in the week prior.")
  tab_counter <- tab_counter+1
  ...
  tab_c5 <- ts_test_lab %>%
            ...
            mutate(pct_change = ifelse(n_res_Previous>0,
                                       ifelse(n_res_Current-n_res_Previous>0,
                                              glue("+{round((n_res_Current - n_res_Previous)/n_res_Previous*100, 1)}%"),
                                              glue("{round((n_res_Current - n_res_Previous)/n_res_Previous*100, 1)}%")),
                                       "-"))
...
```{r Figure-C5-Quar, fig.cap=cap}
  ...
  ggplot(ts_quar_offcamp %>% filter(campus_fac == "UNH Durham"), aes(x=date, y=n)) +
    geom_col(aes(fill=location), width=0.9) +
    theme_bw()+
    scale_fill_brewer("",type = "qual") +
    scale_y_continuous(name = "", breaks=breaks_pretty()) +
    scale_x_date(name="", breaks="1 week", limits = xaxis_entire) +
...
```{r Figure-C6-Quar-7day, fig.cap=cap}
  ...
  labs <- ts_quar_offcamp %>% 
          filter(campus_fac == "UNH Durham"& date > report_date_minus7 & date<=report_date) %>%
          group_by(date) %>%
          summarise(n=sum(n))

  ## number in quarantine, colored by quar location, by day, last 7 days
  ggplot(ts_quar_offcamp %>% filter(campus_fac == "UNH Durham" & date > report_date_minus7 & date<=report_date), aes(x=date, y=n)) +
    geom_col(aes(fill=location), width=0.9)+
    geom_text(data=labs, aes(x=date, y=n+10, label=n)) +
    theme_bw()+
    scale_fill_brewer("",type = "qual") +
    scale_y_continuous(name = "", breaks=breaks_pretty()) +
    scale_x_date(name="", breaks="1 day", limits = xaxis_reporting) +
...
```{r Figure-C7-QuarBeds, fig.cap=cap}
  ...
  cap <- glue("**Figure C{fig_counter}**. Number in quarantine dorm (bed) by day, by residence location, for the entire reporting period.")
  fig_counter <- fig_counter + 1
...
```{r Figure-C8-QuarBeds-7day, fig.cap=cap}
  ...
  ts_quar_campus_tmp <- ts_quar_campus %>% filter(campus_fac == "UNH Durham" & date > report_date_minus7 & date <= report_date)

  label_height = max(c(ts_quar_campus_tmp$nbed, quar_bed_limit), na.rm=TRUE)
  
  ## number quarantine beds in use by day, past 7 days
  ggplot(ts_quar_campus_tmp, aes(x=date, y=nbed)) +
    geom_col(aes(fill=off_campus_fac))+
    geom_label(aes(y=label_height+0.2*label_height/as.numeric(off_campus_fac), label=nbed, fill=off_campus_fac), fontface="bold", show.legend = FALSE) +
    theme_bw()+
    scale_fill_brewer("",palette = "Dark2") +
    scale_y_continuous(name = "", breaks=breaks_pretty()) +
    scale_x_date(name="", breaks="1 day", limits = xaxis_reporting) +
  ...
  cap <- glue("**Figure C{fig_counter}**. Number in quarantine dorm (bed) by day, by residence location, for the reporting week.")
  fig_counter <- fig_counter + 1
```

$~$

```{r Figure-C8B-Quar-Proj, fig.cap=cap}

  ts_quar_proj_tmp <- ts_quar_proj_final %>%
                      filter(campus_fac == "UNH Durham") %>%
                      mutate(scaling_fac_lab = factor(scaling_fac,
                                                      levels = c("0.5", "1", "2 w/o off-campus", "2", "Actual"))) %>%
  ...
  cap <- glue("**Figure C{fig_counter}**. Projected number in quarantine dorm (bed) by day, by typical residence location, under the assumption that 0.5x, 1x, or 2x the amount of observed admissions into quarantine over the reporting period enter isolation in the next 7 days. We also present a scenario in which 2x the number of observed admissions among on-campus students occur in the next 7 days, but there are no off-campus students admitted.")
...
```{r Figure-C10-Isol-7day, fig.cap=cap}
  ...
  labs <- ts_isol_offcamp %>% 
          filter(campus_fac == "UNH Durham"& date > report_date_minus7 & date<=report_date) %>%
          group_by(date) %>%
          summarise(n=sum(n))

  ## number in isolation, colored by isolation location, by day, last 7 days
 ggplot(ts_isol_offcamp %>% filter(campus_fac == "UNH Durham" & date > report_date_minus7 & date <= report_date), aes(x=date, y=n)) +
    geom_col(aes(fill=location), width=0.9)+
    geom_text(data=labs, aes(x=date, y=n+10, label=n)) +
    theme_bw()+
    scale_fill_brewer("",type = "qual") +
...
```{r Figure-C11-IsolBeds, fig.cap=cap}
 ...
 ggplot(ts_isol_campus %>% filter(campus_fac == "UNH Durham"), aes(x=date, y=nbed)) +
  ...
  cap <- glue("**Figure C{fig_counter}**. Number in isolation dorm (bed) by day, by residence location, for the entire reporting period.")
  fig_counter <- fig_counter + 1
...
```{r Figure-C12-IsolBeds-7day, fig.cap=cap}
  ...
  ts_isol_campus_tmp <- ts_isol_campus %>% filter(campus_fac == "UNH Durham" & date > report_date_minus7 & date <= report_date)

  label_height = max(c(ts_isol_campus_tmp$nbed, isol_bed_limit), na.rm=TRUE)
  
  ## number isolation beds in use by day, past 7 days
  ggplot(ts_isol_campus_tmp, aes(x=date, y=nbed)) +
    geom_col(aes(fill=off_campus_fac), width=0.9)+
    geom_label(aes(y=label_height+0.2*label_height/as.numeric(off_campus_fac), label=nbed, fill=off_campus_fac), fontface="bold", show.legend = FALSE) +
    theme_bw()+
    scale_fill_brewer("",palette = "Dark2") +
  ...
  cap <- glue("**Figure C{fig_counter}**. Number in isolation dorm (bed) by day, by residence location, for the reporting week.")
  fig_counter <- fig_counter + 1
...
```{r Figure-C12B-Isol-Proj, fig.cap=cap}
  ...
  cap <- glue("**Figure C{fig_counter}**. Projected number in isolation dorm (bed) by day, by typical residence location, under the assumption that 0.5x, 1x, or 2x the amount of observed admissions into isolation over the reporting period enter isolation in the next 7 days. We also present a scenario in which 2x the number of observed admissions among on-campus students occur in the next 7 days, but there are no off-campus students admitted.")
...
```{r Table-C6-DetailedTable}
  ...
  tab_c6 %>%
    ...
    autofit() %>%
    set_caption(caption=cap) %>%
    hline(i=ind[ind>0], part="body", border=fp_border(width=0.5,color="gray")) 
...
```{r Table-D1-ConcordCond}
  ...
  cap <- glue("Table D{tab_counter}. Concord Campus Operating Conditions. Select indicators are shaded by their operational level.")
  tab_counter <- tab_counter + 1 
...
```{r Table-D3-MostRecentLab, eval=params$concord}
  ...
  tab_d3 <- bind_rows(recent_rep_law, recent_total_law) %>%
            filter(user_status_fac != "Employee") %>%
            mutate(user_status_fac = ifelse(user_status_fac=="Unknown", "Contractors and Visitors", user_status_fac)) %>%
            rename(`Positive` = pos,
                   `Negative` = neg,
...
```{r Table-D5-Lab, eval=params$concord}
  ...
  cap <- glue("Table C{tab_counter}. Laboratory operations: number and percent (of all tests submitted) of tests returned with results at UNH Concord in the reporting period and in the week prior.")  
  tab_counter <- tab_counter+1
...
```{r Figure-D6-Quar-7day, fig.cap=cap, eval=params$concord}
  ...
  ## number in quarantine, colored by quar location, by day, last 7 days
  ggplot(ts_quar_offcamp %>% filter(campus_fac == "UNH Law" & date > report_date_minus7 & date<=report_date), aes(x=date, y=n)) +
    geom_col(aes(fill=location), width=0.9)+
    theme_bw()+
    scale_fill_brewer("",type = "qual") +
    scale_y_continuous(name = "", breaks=breaks_pretty()) +
...
```{r Table-E1-ManchCond}
  ...
  cap <- glue("Table E{tab_counter}. Manchester Campus Operating Conditions. Select indicators are shaded by their operational level.")
  tab_counter <- tab_counter + 1 
...
```{r Table-E3-MostRecentLab, eval=params$manchester}
  ...
  tab_e3 <- bind_rows(recent_rep_man, recent_total_man) %>%
            filter(user_status_fac != "Employee") %>%
            mutate(user_status_fac = ifelse(user_status_fac=="Unknown", "Contractors and Visitors", user_status_fac)) %>%
            rename(`Positive` = pos,
                   `Negative` = neg,
...
```{r Table-E5-Lab, eval=params$manchester}
  ...
  cap <- glue("Table C{tab_counter}. Laboratory operations: number and percent (of all tests submitted) of tests returned with results at UNH Manchester in the reporting period and in the week prior.")
  tab_counter <- tab_counter+1
...
```{r Figure-E6-Quar-7day, fig.cap=cap, eval=params$manchester}
  ...
  ## number in quarantine, colored by quar location, by day, last 7 days
  ggplot(ts_quar_offcamp %>% filter(campus_fac == "UNH Manchester" & date > report_date_minus7 & date<=report_date), aes(x=date, y=n)) +
    geom_col(aes(fill=location), width=0.9)+
    theme_bw()+
    scale_fill_brewer("",type = "qual") +
    scale_y_continuous(name = "", breaks=breaks_pretty()) +
...
```{r Table-E6-DetailedTable, eval=params$manchester}
  ...
  if(nrow(cases_law)==0){ 
        print("No cases in UNH Manchester for reporting period")}else{
                
  cases_man <- cases_man %>% mutate(level = factor(level, levels=c("Total", "Current", "Previous")))
