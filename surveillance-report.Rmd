...
```{r Figure-C4-Map-Active and Cumulative}
...
Allcases <- demo_testing %>% filter(result_fac=="Positive") %>%
        left_join(read_csv("generated_data/linkingfile.csv")) %>%
        group_by(BUILDING_N) %>%
        summarise(cases=n()) 

cases10 <- demo_testing %>% filter(result_fac=="Positive") %>%
        filter(testdate>=(Sys.Date()-10))%>%
        left_join(read_csv("generated_data/linkingfile.csv")) %>%
        group_by(BUILDING_N) %>%
        summarise(cases=n())


dormlist <- demo_testing %>% filter(!is.na(dorm)) %>%
        distinct(dorm) %>%
        left_join(read_csv("generated_data/linkingfile.csv")) %>%
        select(-FACILITIES)
...
dormdf <- bind_rows(left_join(dormdf,Allcases)%>%
                            mutate(map="Cumulative Cases"),
                    left_join(dormdf,cases10)%>%
...
dormdf %>% mutate(caseSize=calc_radius(cases)) %>% 
        ggplot() +
...
```{r Figure-D6-Quar-7day, fig.cap=cap}
...
```{r Figure-D8-Isol-7day, fig.cap=cap}
...
```{r Figure-E6-Quar-7day, fig.cap=cap}
...
```{r Figure-E8-Isol-7day, fig.cap=cap}
