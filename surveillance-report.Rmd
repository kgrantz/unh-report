...
```{r isol-quar-projections}
  ...
  # formatting to add in 'actual' categories
  ts_isol_proj_final <- ts_isol_proj_final %>%
                        filter(date <= report_date) %>%
                        mutate(scaling_fac_label = "Actual") %>%
                        distinct() %>%
...
```{r Figure-C8B-Quar-Proj, fig.cap=cap}
  ...
  ggplot(ts_quar_proj_final %>% filter(campus_fac=="UNH Durham"), aes(x=date, y=nbed, group=scaling_fac_label, col=scaling_fac_label)) +
    geom_line(lwd=1.15)+
  ...
  cap <- glue("**Figure C{fig_counter}**. Projected number in quarantine dorm (bed) by day, by residence location, under the assumption that 0.5x, 1x, or 2x the amount of observed admissions into quarantine over the reporting period enter isolation in the next 7 days.")
  fig_counter <- fig_counter + 1
...
```{r Figure-C12B-Isol-Proj, fig.cap=cap}
  ...
  ggplot(ts_isol_proj_final %>% filter(campus_fac=="UNH Durham"), aes(x=date, y=nbed, group=scaling_fac_label, col=scaling_fac_label)) +
    geom_line(lwd=1.15)+
    facet_wrap(.~off_campus_fac)+
    theme_bw()+
    scale_color_brewer("",palette="Dark2") +
  ...
  cap <- glue("**Figure C{fig_counter}**. Projected number in isolation dorm (bed) by day, by residence location, under the assumption that 0.5x, 1x, or 2x the amount of observed admissions into isolation over the reporting period enter isolation in the next 7 days.")
