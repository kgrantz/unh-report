...
params:
  ...
  report_date: "2021-02-03" # if "AUTO", takes Sys.Date()-2; otherwise specify as YYYY-MM-DD
...
```{r Figure-C8B-Quar-Proj, fig.cap=cap}
  ...
  ts_quar_proj_tmp <- ts_quar_proj_final %>%
                      ...
                      mutate(scaling_fac_lab = factor(scaling_fac,
                                                      levels = c("0.5", "1", "4 w/o off-campus", "4", "Actual"))) %>%
  ...
  cap <- glue("**Figure C{fig_counter}**. Projected number in quarantine dorm (bed) by day, by typical residence location, under the assumption that 0.5x, 1x, or 4x the amount of observed admissions into quarantine over the reporting period enter isolation in the next 7 days. We also present a scenario in which 4x the number of observed admissions among on-campus students occur in the next 7 days, but there are no off-campus students admitted.")
...
```{r Figure-C12B-Isol-Proj, fig.cap=cap}
  ...
  cap <- glue("**Figure C{fig_counter}**. Projected number in isolation dorm (bed) by day, by typical residence location, under the assumption that 0.5x, 1x, or 4x the amount of observed admissions into isolation over the reporting period enter isolation in the next 7 days. We also present a scenario in which 4x the number of observed admissions among on-campus students occur in the next 7 days, but there are no off-campus students admitted.")
