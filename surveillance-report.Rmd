...
params:
  ...
  report_date: "2020-11-13" # date in YYYY-MM-DD
...
* The number of *statewide* cases continue to rise above and beyond the red trigger threshold. The number of hospitalizations has increased 25% but remains in the orange.


**Campus-wide**

* Though this week’s percent positivity has increased since last week, the number remains in the yellow trigger threshold.


**At Durham**

* The number of active cases has switched from the yellow to orange trigger threshold. 

* There was a 633% increase in the number of active cases in dorms — distributed throughout across living areas on campus. There is a somewhat larger number in the Timbers (n=25) currently, as compared to other living areas. Off-campus cases continue to rise at a similar rate as the last reporting period.

* Increases in cases were greatest in the Business school (337% increase) as well as the liberal arts and Engineering (both 166% increases).

* Though the largest number of cases occurred among students, faculty is also seeing increases that appear to parallel statewide conditions.

* The number of isolation beds in use has jumped from yellow to red. Cases may need to begin sharing rooms in the isolation dorm.

* Quarantine beds on campus remain at less than half of capacity.
...
```{r Table-B1B-CampusCond}
  ...
  indicatordf <-  left_join(testdf,casedf,by="campus_fac")  %>%
                  ...
                  filter(!(campus_fac %in% c("Other","No Campus Listed"))) %>%
...
```{r Figure-C8-QuarBeds-7day, fig.cap=cap}
  ...
  ggplot(ts_quar_campus %>% filter(campus_fac == "UNH Durham" & date > report_date_minus7 & date <= report_date), aes(x=date, y=nbed)) +
    geom_col(width=0.9, fill="#120075")+
    geom_text(aes(y=nbed+10, label=nbed)) +
    theme_bw()+
    scale_fill_brewer("",type = "qual") +
    scale_y_continuous(name = "", breaks=breaks_pretty()) +
    scale_x_date(name="", breaks="1 day", limits = xaxis_reporting) +
...
```{r Figure-C12-IsolBeds-7day, fig.cap=cap}
 ...
 ggplot(ts_isol_campus %>% filter(campus_fac == "UNH Durham" & date > report_date_minus7 & date <= report_date), aes(x=date, y=nbed)) +
    geom_col(width=0.9, fill="#120075")+
    geom_text(aes(y=nbed+5, label=nbed)) +
    theme_bw()+
    scale_fill_brewer("",type = "qual") +
    scale_y_continuous(name = "", breaks=breaks_pretty()) +
    scale_x_date(name="", breaks="1 day", limits = xaxis_reporting) +
